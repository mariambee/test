...
char* indexes(const char* path)
{
    ...
    if (stat(path, &sb) == 0 && S_ISDIR(sb.st_mode))
    {
        ...
        // allocate enough space for the concatenation of path and "index.html"
        buffer = malloc ((sizeof(char) * strlen(path)) + (11 * sizeof(char)));
        // check for NULL
        if (buffer == NULL)
        {
            return NULL;
        }
        // copy path into that space
        strcpy(buffer, path);
        // append "index.html"
        ...
        // allocate enough space for the concatenation of path and "index.html"
        buffer = malloc ((sizeof(char) * strlen(path)) + (10 * sizeof(char)));
        // check for NULL
        if (buffer == NULL)
        {
            return NULL;
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    while (!feof(file))
    {
        // create size to store amount of bytes from read
        size_t size = fread(&buffer, sizeof(BYTE), BYTES, file);
        // reallocate memory
        *content = realloc(*content, *length + size);
        // check for NULL
        if (*content == NULL)
        {
            return false;
        }
        // copy new bytes into content 
        memcpy(*content + *length, buffer, size);
        // increment length
        *length += size;
    }
    return true;
...
const char* lookup(const char* path)
{
    // find '.'
    char* s = strrchr(path, '.');
    // then compare and return accordingly
    if (strcmp(s, ".css") == 0)
    {
        return "text/css";
    }
...
{
    // check there are only 2 spaces and the line ends in "/r/n"
    int lineindex = 0;
    int spaces = 0;
    bool endline = false;
    int len = strlen(line);
    while (lineindex < len )
    {
        ...
        if (line[lineindex + 1] == '\r' && line[lineindex + 2] == '\n')
        {
            endline = true;
        }
        lineindex++;
    }
    if (spaces != 2 || endline == false)
    {
        return false;
    }
    ...
    // find pointer to first occurance of space
    char* firstspace = strchr(line, ' ');
    // move pointer over one
    firstspace += 1;
    // check if the first character after the space is a /
    if (firstspace[0] != '/')
    {
        error(501);
        return false;
    }
    // check if request-target contains a "
    if (strchr(firstspace, '"') != NULL)
    {
        error(400);
        return false;
    }
    ...
    if (strncmp(secondspace, " HTTP/1.1\r\n", 11) != 0)
    {
        error(505);
        return false;
    }
    // store request-target into abs_path
    int i;
    for (i = 0; !((firstspace[i] == '?') || (firstspace[i] == ' ')); i++)
    {
        abs_path[i] = firstspace[i];
    }
    // add null terminator
    abs_path[i] = '\0';
    // find the "?" for the beginning of the query
    char* qquestion = strchr(line, '?');
    // store the query if present
    if (qquestion != NULL)
    {
        int i;
        for (i = 0; qquestion[i + 1] != ' '; i++)
        {
            query[i] = qquestion[i + 1];
        }
        // add null terminator
        query[i + 1] = '\0';
    }
